...
import {
    ...
    Globe, EyeOff, Users, MessageSquare, Filter
} from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                ...
                <div className="flex flex-col items-center text-center gap-4">
                    <div className={`w-14 h-14 rounded-full flex items-center justify-center ${confirmColor.includes('red') ? 'bg-red-100' : 'bg-amber-100'}`}>
                    ...
                    <div>
                        ...
                        <p className="text-neutral-500 text-sm mt-1.5 leading-relaxed">{desc}</p>
                    </div>
...
export default function AdminWeddingsPage() {
    ...
    const themeColors: any = {
        javanese: 'bg-amber-500/10 text-amber-600 border-amber-500/20',
        minimalist: 'bg-blue-500/10 text-blue-600 border-blue-500/20',
        elegant: 'bg-purple-500/10 text-purple-600 border-purple-500/20',
    };
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2 bg-white shadow-sm border border-neutral-200 rounded-xl px-4 py-2.5 flex-1 min-w-[200px] max-w-sm">
                ...
                <div className="flex items-center bg-neutral-100 rounded-xl p-1 gap-1">
                    ...
                    {([['', 'Semua'], ['true', 'Publik'], ['false', 'Draft']] as const).map(([val, label]) => (
                        <button key={val} onClick={() => { setFilterPublished(val); setPage(1); }}
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${filterPublished === val ? 'bg-white shadow text-neutral-800' : 'text-neutral-400 hover:text-neutral-600'}`}>
                            {label}
                        </button>
                    ))}
                </div>

                <button onClick={fetchWeddings} className="p-2.5 bg-white border shadow-sm border-neutral-200 rounded-xl text-neutral-500 hover:text-gold transition-colors">
                    <RefreshCw className="w-4 h-4" />
                </button>
                <span className="text-neutral-500 text-sm ml-auto">{total} undangan</span>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-20 text-neutral-500">Tidak ada undangan ditemukan</div>
            ) : (
                    ...
                    <AnimatePresence>
                        {weddings.map((w, i) => (
                            <motion.div key={w.id} layout initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                className="bg-white shadow-sm border border-neutral-200 rounded-2xl p-5 hover:border-neutral-300 transition-colors group">

                                {/* Header */}
                                <div className="flex items-start justify-between mb-3">
                                    <div className="min-w-0 flex-1">
                                        <h3 className="text-neutral-900 font-bold truncate">{w.groomShort} & {w.brideShort}</h3>
                                        <p className="text-neutral-400 text-xs mt-0.5 font-mono truncate">/{w.slug}</p>
                                    </div>
                                    <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full border shrink-0 ml-2 ${themeColors[w.themeId] || 'bg-neutral-100 text-neutral-500 border-neutral-200'}`}>
                                        {w.themeId}
                                    </span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 pt-3 border-t border-neutral-100">
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all ${w.isPublished
                                            ...
                                            : 'bg-neutral-100 text-neutral-500 hover:bg-emerald-100 hover:text-emerald-600'}`}
                                    >
                                        {togglingId === w.id
                                    ...
                                    <Link href={`/${w.slug}`} target="_blank"
                                        className="p-1.5 rounded-lg text-neutral-400 hover:text-blue-500 hover:bg-blue-500/10 transition-all" title="Lihat">
                                        <Eye className="w-4 h-4" />
                                    </Link>
                                    <button onClick={() => setDeleteTarget(w)}
                                        className="p-1.5 rounded-lg text-neutral-400 hover:text-red-500 hover:bg-red-500/10 transition-all" title="Hapus">
                ...
                <div className="flex justify-center gap-2">
                    {Array.from({ length: Math.ceil(total / 20) }, (_, i) => i + 1).map(p => (
                        <button key={p} onClick={() => setPage(p)}
                            className={`w-9 h-9 rounded-xl text-sm font-medium transition-all ${page === p ? 'bg-gold text-primary' : 'bg-white shadow-sm border border-neutral-200 text-neutral-500 hover:text-neutral-900'}`}>
                            {p}
                        </button>
                    ))}
                </div>
